...
<div class="row justify-content-center">
    ...
    <input type="hidden" name="std_id" value="{{ Session::get('std_id') }}">
    @if(\App\Model\Points::checkPoint() > 0 && empty(Session::get('res')))
        <div class="col-md-12">
    ...
    @else
        @php
            $status = (!empty(Session::get('res')) ? 'expire_class' : 'no_point');
        @endphp
        <div class="col-md-5">
            <div class="card">
                <div class="card-header bg-warning">
                    <h3 class="m-0 text-white">@if($status == 'no_point')Point @else Class @endif</h3>
                </div>
                <div class="card-body text-center">
                    <h4 class="badge badge-info font-16">@if($status == 'no_point') You have 0 point @else Class Expired @endif</h4>
                    <h4>Please contact customer service</h4>
                </div>
            </div>
        </div>
    @endif
