...
class KTC extends Model
{
    public static function data_list($pag)
    {
        $ktc_order = DB::table('ktc_order')
            ->select('ktc_order.id', 'ktc_order.order_id', 'ktc_order.success_code', 'ktc_order.order_ref', 'ktc_order.created_at', 'users.first_name', 'users.last_name', 'users.status', 'users.email', 'users.address')
